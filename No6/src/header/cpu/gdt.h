 ...
 * @param privilege          2-bit contain privilege level
 * @param segment_present   1-bit contain whether segment is present in memory or not
 * @param segment_high     4-bit contain segment limit
 * @param avl     1-bit contain avaiable-for-use by system software
 * @param long_mode            1-bit contain 61-bit code segment
 * @param opr_32_bit           1-bit contain default operation size
 * @param granularity  1-bit contain granularity flag
...
struct SegmentDescriptor {
    ...
    uint8_t base_mid;
    uint8_t type_bit   : 4; //4
    uint8_t non_system : 1;
    uint8_t privilege : 2; //2
