#include "header/driver/cmos.h"

unsigned char second;
unsigned char minute;
unsigned char hour;
unsigned char day;
unsigned char month;
unsigned int year;

void out_byte(int port, int value) {
    asm volatile(
        "outb %%al, %%dx"
        :
        : "a"(value), "d"(port)
    );
}

int in_byte(int port) {
    int ret;
    asm volatile(
        "inb %%dx, %%al"
        : "=a"(ret)
        : "d"(port)
    );
    return ret;
}

enum {
      cmos_address = 0x70,
      cmos_data    = 0x71
};
 
int get_update_in_progress_flag() {
    out_byte(cmos_address, 0x0A); // Select register A
    if (in_byte(cmos_data) & 0x80) {
        return 1; // Update in progress
    }
    return (in_byte(cmos_data) & 0x80);
}
 
unsigned char get_RTC_register(int reg) {
    out_byte(cmos_address, reg);
    return in_byte(cmos_data);
}

void read_rtc(unsigned char *hour_ptr, unsigned char *minute_ptr, unsigned char *second_ptr) {
    while (get_update_in_progress_flag()); // Wait for update to complete

    second = get_RTC_register(0x00);
    minute = get_RTC_register(0x02);
    hour   = get_RTC_register(0x04);
    day    = get_RTC_register(0x07);
    month  = get_RTC_register(0x08);
    year   = get_RTC_register(0x09);

    unsigned char last_second = second;
    unsigned char last_minute = minute;
    unsigned char last_hour   = hour;
    unsigned char last_day    = day;
    unsigned char last_month  = month;
    unsigned char last_year   = year;

    do{

        while (get_update_in_progress_flag()); // Wait for update to complete

        second = get_RTC_register(0x00);
        minute = get_RTC_register(0x02);
        hour   = get_RTC_register(0x04);
        day    = get_RTC_register(0x07);
        month  = get_RTC_register(0x08);
        year   = get_RTC_register(0x09);
    } while ((last_second != second) || (last_minute != minute) || (last_hour != hour) || 
          (last_day != day) || (last_month != month) || (last_year != year));

    unsigned char register_b = get_RTC_register(0x0B);

    // Konversi dari BCD (Binary Coded Decimal) ke Binary jika diperlukan
    if (!(register_b & 0x04)) {
        second = (second & 0x0F) + ((second / 16) * 10);
        minute = (minute & 0x0F) + ((minute / 16) * 10);
        hour   = ( (hour & 0x0F) + (((hour & 0x70) / 16) * 10) ) | (hour & 0x80);
        day    = (day & 0x0F) + ((day / 16) * 10);
        month  = (month & 0x0F) + ((month / 16) * 10);
        year   = (year & 0x0F) + ((year / 16) * 10);
    }

    // Konversi dari format 12 jam ke 24 jam jika diperlukan
    if (!(register_b & 0x02) && (hour & 0x80)) {
        hour = ((hour & 0x7F) + 12) % 24;
    }

    // Hitung tahun penuh (misal 25 menjadi 2025)
    if(year < (CURRENT_YEAR % 100)) {
        year += (CURRENT_YEAR / 100) * 100;
    } else {
        year += (CURRENT_YEAR / 100 - 1) * 100;
    }

    // Set nilai pointer yang diberikan
    *hour_ptr   = hour;
    *minute_ptr = minute;
    *second_ptr = second;
}
