...
void init_directory_table(struct FAT32DirectoryTable *dir_table, char *name __attribute__((unused)), uint32_t parent_dir_cluster) {
...
void create_fat32(void){
    ...
    // Initialize the FAT32 file allocation table
    ...
    // Cluster allocation for root directory
    fat.cluster_map[FAT_CLUSTER_NUMBER] = FAT32_FAT_END_OF_FILE;
    write_clusters(&fat, FAT_CLUSTER_NUMBER, 1);

    // Initialize the root directory
    struct FAT32DirectoryTable root_dir = fat32_driver_state.dir_table_buf;
...
bool get_dir_table_from_cluster(uint32_t cluster, struct FAT32DirectoryTable *dir_entry){
    if (cluster < 2 || cluster >= CLUSTER_MAP_SIZE) {
        return false; // Invalid cluster number
    }
